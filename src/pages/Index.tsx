...
const Index = () => {
  const [currentView, setCurrentView] = useState<'landing' | 'userSelection' | 'registration' | 'clientDashboard' | 'advocateDashboard' | 'internDashboard'>('landing');
  const [selectedUserType, setSelectedUserType] = useState<'client' | 'advocate' | 'intern' | null>(null);

  const handleUserTypeSelect = (type: 'client' | 'advocate' | 'intern') => {
  ...
  const handleRegistrationComplete = () => {
    ...
    } else if (selectedUserType === 'advocate') {
      setCurrentView('advocateDashboard');
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <Card className="border-slate-200 hover:shadow-xl transition-all duration-300 hover:scale-105">
              <CardHeader className="text-center">
                ...
                <CardDescription>Gain experience in the legal field</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <ul className="space-y-2 text-sm text-slate-600">
                  <li>• Find internship opportunities</li>
                  <li>• Connect with advocates</li>
